...
test_that("mask values are not numeric", {
  ...
  testRas <- raster::raster(xmn = -5, xmx = 5, ymn = -5, ymx = 5, res = c(1,1))
  testMask <- testRas
  testRas <- raster::setValues(testRas, round(rnorm(100), digits = 0))
  testMask <- raster::setValues(testMask, round(rnorm(100), digits = 0))
